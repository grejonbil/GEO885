...
emission_distance_plot <- ggplot()+
  geom_point(data = amm, aes(y= kgCO2km, x = distance_km, col="yellow"), size = 1)+
  geom_point(data = amm_economy, aes(y= kgCO2km, x = distance_km, col="green"), size = .4)+
  geom_vline(xintercept = 1000, linetype="dotted", size=.5)+
  scale_color_manual(labels = c("Economy modeled", "Observed"),
                     values = c("#00BA38", "#D55E00"))+
  annotate("text", x = 3000, y = 1, label=label1, color=black, size=3) + 
...
```{r}
nrow(amm)
#5463 (im paper andere Zahl?)

nrow(amm_shortdist)
#2601 Kurzflüge

nrow(amm_shortdist)/nrow(amm)
#0.476 --> knapp 48% Kurzflüge 
